    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['scores:staticticsclass:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['scores:staticticsclass:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['scores:staticticsclass:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listStaticticsclass, getStaticticsclass, delStaticticsclass, addStaticticsclass, updateStaticticsclass } from "@/api/scores/staticticsclass";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStaticticsclass(this.queryParams).then(response => {
        this.staticticsclassList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getStaticticsclass(examStatisticsId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.examStatisticsId != null) {
            updateStaticticsclass(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStaticticsclass(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除班级成绩统计情况编号为"' + examStatisticsIds + '"的数据项？').then(function() {
        return delStaticticsclass(examStatisticsIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('scores/staticticsclass/export', {
        ...this.queryParams
      }, `staticticsclass_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
